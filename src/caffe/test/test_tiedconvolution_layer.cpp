...
// Exactly the same as the one in test_convolution_layer.cpp
            ...
            for (int x = 0; x < out->width(); x++) {
              for (int p = 0; p < kernel_h; p++) {
                for (int q = 0; q < kernel_w; q++) {
                  ...
                  if (in_y >= 0 && in_y < in->height() && in_x >= 0 &&
                      in_x < in->width()) {
...
class TiedConvolutionLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

protected:
  TiedConvolutionLayerTest()
      : blob_bottom_1(new Blob<Dtype>(2, 3, 6, 4)),
        blob_top_1(new Blob<Dtype>()),
        blob_bottom_2(new Blob<Dtype>(2, 3, 5, 7)),
  ...
  virtual void SetUp() {
    ...
    blob_bottom_vec_.push_back(blob_bottom_2);
  ...
  virtual ~TiedConvolutionLayerTest() {
...
TYPED_TEST_CASE(TiedConvolutionLayerTest, TestDtypesAndDevices);

TYPED_TEST(TiedConvolutionLayerTest, TestSetup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter *convolution_param =
      layer_param.mutable_convolution_param();
  ...
  convolution_param->set_stride(2);
  convolution_param->set_num_output(4);
  shared_ptr<TiedConvolutionLayer<Dtype> > layer(
      new TiedConvolutionLayer<Dtype>(layer_param));
  layer->LayerSetUp(this->blob_bottom_vec_, &(this->blob_top_vec_));
  layer->Reshape(this->blob_bottom_vec_, &(this->blob_top_vec_));

  EXPECT_EQ(layer->num_in(), 2);

  EXPECT_EQ(this->blob_top_1->num(), 2);
  EXPECT_EQ(this->blob_top_1->channels(), 4);
  EXPECT_EQ(this->blob_top_1->height(), 2);
  EXPECT_EQ(this->blob_top_1->width(), 1);

  EXPECT_EQ(this->blob_top_2->num(), 2);
  EXPECT_EQ(this->blob_top_2->channels(), 4);
  EXPECT_EQ(this->blob_top_2->height(), 2);
  EXPECT_EQ(this->blob_top_2->width(), 3);
  // setting group should not change the shape
  convolution_param->set_num_output(3);
  convolution_param->set_group(3);
  layer.reset(new TiedConvolutionLayer<Dtype>(layer_param));
  layer->LayerSetUp(this->blob_bottom_vec_, &(this->blob_top_vec_));
  layer->Reshape(this->blob_bottom_vec_, &(this->blob_top_vec_));

  EXPECT_EQ(this->blob_top_1->num(), 2);
  EXPECT_EQ(this->blob_top_1->channels(), 3);
  EXPECT_EQ(this->blob_top_1->height(), 2);
  EXPECT_EQ(this->blob_top_1->width(), 1);
  EXPECT_EQ(this->blob_top_2->num(), 2);
  EXPECT_EQ(this->blob_top_2->channels(), 3);
  EXPECT_EQ(this->blob_top_2->height(), 2);
  EXPECT_EQ(this->blob_top_2->width(), 3);
}

TYPED_TEST(TiedConvolutionLayerTest, TestSimpleConvolution) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter *convolution_param =
      layer_param.mutable_convolution_param();
  ...
  shared_ptr<Layer<Dtype> > layer(
      new TiedConvolutionLayer<Dtype>(this->layer_param));
  layer->SetUp(this->blob_bottom_vec_, &(this->blob_top_vec_));
  layer->Forward(this->blob_bottom_vec_, &(this->blob_top_vec_));
  const Dtype *top_data_1 = this->blob_top_1->cpu_data();
  const Dtype *top_data_2 = this->blob_top_2->cpu_data();
  // Check against reference convolution.
  caffe_conv(this->blob_bottom_1, convolution_param, layer->blobs(),
             this->MakeReferenceTop(this->blob_top_1));
  const Dtype *ref_top_data = this->ref_blob_top_->cpu_data();
  for (int i = 0; i < this->blob_top_1->count(); ++i) {
    EXPECT_NEAR(top_data_1[i], ref_top_data[i], 1e-4);
  }
  caffe_conv(this->blob_bottom_2, convolution_param, layer->blobs(),
             this->MakeReferenceTop(this->blob_top_2));
  ref_top_data = this->ref_blob_top_->cpu_data();
  for (int i = 0; i < this->blob_top_2->count(); ++i) {
    EXPECT_NEAR(top_data_2[i], ref_top_data[i], 1e-4);
  }
}

TYPED_TEST(TiedConvolutionLayerTest, TestSimpleConvolutionGroup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter *convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(TiedConvolutionLayerTest, TestSimpleConvolutionWithPad) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter *convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(TiedConvolutionLayerTest, TestSobelConvolution) {
  ...
  filler->Fill(this->blob_bottom_1);
  this->blob_bottom_2->ReshapeLike(*this->blob_bottom_1);
  this->blob_bottom_2->CopyFrom(*this->blob_bottom_1);
  ...
  shared_ptr<Layer<Dtype> > layer(new TiedConvolutionLayer<Dtype>(layer_param));
  ...
  sep_blob_bottom_vec.push_back(this->blob_bottom_2);
  sep_blob_top_vec.push_back(this->blob_top_2);
  ...
  blob_sep->CopyFrom(*this->blob_top_2, false, true);
  ...
  const Dtype *top_data = this->blob_top_1->cpu_data();
  const Dtype *sep_top_data = this->blob_top_2->cpu_data();
  for (int i = 0; i < this->blob_top_1->count(); ++i) {
    EXPECT_NEAR(top_data[i], sep_top_data[i], 1e-4);
  }
}

TYPED_TEST(TiedConvolutionLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_1->Reshape(2, 2, 3, 3);
  this->blob_bottom_2->Reshape(2, 2, 9, 9);
  ...
  ConvolutionParameter *convolution_param =
      this->layer_param.mutable_convolution_param();
  convolution_param->set_kernel_size(3);
  convolution_param->set_stride(2);
  ...
  TiedConvolutionLayer<Dtype> layer(this->layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
  checker.CheckGradientExhaustive(&layer, &(this->blob_bottom_vec_),
                                  &(this->blob_top_vec_));
}

TYPED_TEST(TiedConvolutionLayerTest, TestGradientGroup) {
  typedef typename TypeParam::Dtype Dtype;
