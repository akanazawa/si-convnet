...
void CCCPPoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype> *> &bottom,
  ...
  biasterm_ = this->layer_param_.convolution_param().bias_term();
  ...
  CHECK_EQ(CHANNEL_ % GROUP_, 0)
      << "CCCP Pooling input channel number is not divisible by group number.";

  (*top)[0]->Reshape(bottom[0]->num(), GROUP_ * NUM_OUTPUT_,
  ...
  if (this->blobs_.size() > 0) {
    ...
    this->blobs_[0].reset(
        new Blob<Dtype>(1, 1, GROUP_ * NUM_OUTPUT_, CHANNEL_ / GROUP_));
    // fill the weights
  ...
  for (int n = 0; n < NUM_; ++n) {
    for (int g = 0; g < GROUP_; ++g) {
      caffe_cpu_gemm<Dtype>(
          CblasNoTrans, CblasNoTrans, NUM_OUTPUT_, REST_, CHANNEL_ / GROUP_,
    ...
    if (biasterm_) {
      caffe_cpu_gemm<Dtype>(
          CblasNoTrans, CblasNoTrans, GROUP_ * NUM_OUTPUT_, REST_, 1, (Dtype)1.,
...
void CCCPPoolingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
  ...
  if (biasterm_) {
    ...
    for (int n = 0; n < NUM_; ++n) {
      caffe_cpu_gemv<Dtype>(
          CblasNoTrans, NUM_OUTPUT_ * GROUP_, REST_, (Dtype)1.,
  ...
  for (int n = 0; n < NUM_; ++n) {
    ...
    for (int g = 0; g < GROUP_; ++g) {
      ...
      caffe_cpu_gemm<Dtype>(
          CblasNoTrans, CblasTrans, NUM_OUTPUT_, CHANNEL_ / GROUP_, REST_,
          (Dtype)1., top_diff + top[0]->offset(n) + g * top_group_offset,
          bottom_data + (*bottom)[0]->offset(n) + g * bottom_group_offset,
      ...
      if (propagate_down[0]) {
        caffe_cpu_gemm<Dtype>(
            CblasTrans, CblasNoTrans, CHANNEL_ / GROUP_, REST_, NUM_OUTPUT_,
            (Dtype)1., weight + g * weight_offset,
...
void CCCPPoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top,
  ...
  if (biasterm_) {
    ...
    CUDA_CHECK(
        cudaMemset(bias_diff, 0, sizeof(Dtype) * this->blobs_[1]->count()));
    for (int n = 0; n < NUM_; ++n) {
