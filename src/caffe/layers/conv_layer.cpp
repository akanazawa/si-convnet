...
void ConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
  ...
  col_buffer_.Reshape(1, channels_ * kernel_h_ * kernel_w_, height_out_,
                      width_out_);
  for (int top_id = 0; top_id < top->size(); ++top_id) {
    (*top)[top_id]->Reshape(num_, num_output_, height_out_, width_out_);
  }
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < num_; ++n) {
      ...
      for (int g = 0; g < group_; ++g) {
        // AJ: calls cblas_sgemm(rowmajor, transA, TransB, M_, N_, K_, 1,
        // Weight,
        // K_, X, N_, 0, top of this image)
        // i.e. with weight = M_ x K_, col_data = K_ x N_
        // out = W*col_data,
      ...
      // Add bias.
      if (bias_term_) {
        caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_output_, N_, 1,
...
void ConvolutionLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    if (bias_term_ && this->param_propagate_down_[1]) {
      ...
      for (int n = 0; n < num_; ++n) {
        caffe_cpu_gemv<Dtype>(CblasNoTrans, num_output_, N_, 1.,
    ...
    if (this->param_propagate_down_[0] || propagate_down[i]) {
      ...
      for (int n = 0; n < num_; ++n) {
        ...
        if (propagate_down[i]) {
          ...
          col2im_cpu(col_diff, channels_, height_, width_, kernel_h_, kernel_w_,
...
template <typename Dtype>
void ConvolutionLayer<Dtype>::Report(const std::string &name) {
  // montage(this->blobs_[0].get(), name + " conv");
  // montage(this->blobs_[0].get(), name + " conv bprop", true);
  // cv::waitKey(0);
  // const Dtype *weight = this->blobs_[0]->cpu_data();
  // Dtype mean = 0;
  // for (int i = 0; i < this->blobs_[0]->count(); ++i) {
  //   mean += weight[i];
  //   if (weight[i] != weight[i] || std::isinf(weight[i]))
  //     LOG(FATAL) << "nan in the weight!" << weight[i] << " i:" << i;
  // }

  // LOG(INFO) << "mean weight " << mean / this->blobs_[0]->count();
  // mean = 0.;
  // for (int i = 0; i < this->blobs_[1]->count(); ++i) {
  //   mean += this->blobs_[1]->cpu_data()[i];
  //   if (this->blobs_[1]->cpu_data() != this->blobs_[1]->cpu_data())
  //     LOG(FATAL) << "nan found in bias!" << i;
  // }
  // LOG(INFO) << "mean bias " << mean / this->blobs_[1]->count();
}
