...
Net<Dtype>::Net(const NetParameter& param, bool quiet) {
  Init(param, quiet);
}
...
void Net<Dtype>::Init(const NetParameter& in_param, bool quiet) {
  ...
  for (int layer_id = 0; layer_id < param.layers_size(); ++layer_id) {
    ...
    LOG_IF(INFO, !quiet) << "Creating Layer " << layer_param.name();
    ...
    for (int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id) {
      ...
      LOG_IF(INFO, !quiet) << "Top shape: "
                           << top_vecs_[layer_id][top_id]->num() << " "
    ...
    if (need_backward) {
      // LOG_IF(INFO, !quiet) << layer_names_[layer_id] << " needs backward
      // computation.";
  ...
  for (int layer_id = layers_.size() - 1; layer_id >= 0; --layer_id) {
    ...
    if (layer_need_backward_[layer_id]) {
      ...
      LOG_IF(INFO, !quiet) << layer_names_[layer_id]
  ...
  }// AJ: END of setting up layers & their connections

  // In the end, all remaining blobs are considered output blobs.
  for (set<string>::iterator it = available_blobs.begin();
      it != available_blobs.end(); ++it) {
    LOG_IF(INFO, !quiet) << "This network produces output " << *it;
...
void Net<Dtype>::GetLearningRateAndWeightDecay() {
  LOG_IF(INFO, Caffe::phase() == Caffe::TRAIN)
...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    if ( top_vecs_[i].size() > 0 ) {
      const Dtype* top_data = top_vecs_[i][0]->cpu_data();
      for (int j = 0; j < top_vecs_[i][0]->count(); ++j) {
    	CHECK(top_data[j] == top_data[j]) << "nan in top at "
					  << layer_names_[i];
      }
    }
...
template <typename Dtype>
void Net<Dtype>::Report() {
  for (int i = 0; i < layers_.size(); ++i) {
    if (param_owners_[i] > 0) { continue; }
    layers_[i]->Report(layer_names_[i]);
    // if ( layers_[i]->blobs().size() > 0 ) {
    //   float mean_act = 0, max_act = 0, mean_w = 0, max_w = 0;
    //   Blob<Dtype>* blob = top_vecs_[i][0];
    //   const Dtype* data_cpu = blob->cpu_data();
    //   for (int j = 0; j < blob->count(); ++j ) {
    // 	mean_act += data_cpu[j];
    // 	if (data_cpu[j] > max_act) max_act = data_cpu[j];
    // 	if ( isinf(data_cpu[j]) )
    // 	  LOG(ERROR) << "@ " << layer_names_[i] << " " << j << " inf found: " << data_cpu[j];
    //   }
    //   mean_act /= blob->count();
    //   Blob<Dtype>* weight = layers_[i]->blobs()[0].get();
    //   data_cpu = weight->cpu_data();
    //   for (int j = 0; j < weight->count(); ++j ) {
    // 	mean_w += data_cpu[j];
    // 	if (data_cpu[j] > max_w) max_w = data_cpu[j];
    // 	if ( isinf(data_cpu[j]) )
    // 	  LOG(ERROR) << "@ " << layer_names_[i] << " " << j << " inf found: " << data_cpu[j];
    //   }
    //   mean_w /= weight->count();

    //   LOG(INFO) << layer_names_[i] << " mean act: " << mean_act << " max act: " << max_act << " mean weight: " << mean_w << " max weight : " << max_w;

      // Thrust way:
      // const thrust::device_ptr<float> d_ptr(blob->mutable_gpu_data());
      // thrust::device_vector<float> vec(d_ptr, d_ptr + blob->count());
      // thrust::device_vector<float>::iterator max_iter = thrust::max_element(vec.begin(), vec.end());
      // float mean = thrust::reduce(vec.begin(), vec.end(), 0, thrust::plus<float>()) / blob->count();
      // float max_act = *max_iter;
    // }
  }
}
