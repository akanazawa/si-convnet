...
public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_UP;
  }
  virtual inline int MinBottomBlobs() const { return 1; }
  virtual inline int MinTopBlobs() const { return 1; }
...
public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_DOWNPOOL;
  }
  virtual inline int MinBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
...
// and Down
template <typename Dtype> class TIConvolutionLayer : public Layer<Dtype> {
public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_TICONV;
  }
...
// product layer.
// If the underlying layer of this layer is a convolutional layer, then the
// feature
// maps are divided evenly into groups of size "group". Each group of feature
// map
// perform cross channel parameteric pooling and generates "num_output" new
// feature
// maps. So in total this layer outputs group x num_output feature maps.
// If the underlying layer of this layer is another CCCP, then this layer
// ignores
// the grouping of the previous layer and treat all the feature maps from the
// previous layer as a whole, and redo grouping according to its own group
// parameter.
// To respect the grouping from previous layer, just set the group parameter to
// the
// same value as the previous layer, which is equivalent to the unshared mdoe of
// mlpconv.
// CCCP Pooling layer is very flexible in creating shared, unshared, partially
// shared
// types of mlpconv layers.
template <typename Dtype> class CCCPPoolingLayer : public Layer<Dtype> {
public:
