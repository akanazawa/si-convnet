...
class Blob {
 public:
  Blob()
	 ...
	 count_(0), allocateDiff_(true), capacity_(0) {}
  Blob(const bool allocateDiff)
    : data_(), diff_(), num_(0), channels_(0), height_(0), width_(0),
      count_(0), allocateDiff_(allocateDiff), capacity_(0) {}
  explicit Blob(const int num, const int channels, const int height,
		const int width, const bool allocateDiff = true);
  /**
  ...
  inline int allocateDiff() const { return allocateDiff_; }

  // AJ: Data is in B C 0 1 in theano notation or
  // num of images, num of channels, y, x (reading image row order)
