...
class Net {
 public:
  explicit Net(const NetParameter& param, bool quiet = false);
  explicit Net(const string& param_file);
  virtual ~Net() {}

  /// @brief Initialize a network with a NetParameter.
  void Init(const NetParameter& param, bool quiet = false);
  ...
  // Each layer reports any usage/information they want
  void Report();
 ...
 protected:
  ...
  // layers.
  // AJ: name is slightly confusing, these are the neurons, actiations and error signals
  ...
  // pointers.
  // AJ: this is vector of vector, bc first vector corresponds to each layer.
  // The second vector is there bc some layers (like data) returns multiple tops & bottoms
