  ...
  for (int blob_id = 0; blob_id < blobs_to_check.size(); ++blob_id) {
    ...
    for (int feat_id = 0; feat_id < current_blob->count(); ++feat_id) {
      ...
      if (kink_ - kink_range_ > fabs(feature)
        ...
        // LOG(INFO) << "debug: (top_id, top_data_id, blob_id, feat_id)="
	// 	  << top_id << "," << top_data_id << "," << blobid << ","
	// 	  << feat_id << "(actual, est)=" << computed_gradient << ","
	// 	  << estimated_gradient + threshold_*scale;
      }
...
void GradientChecker<Dtype>::CheckGradientExhaustive(Layer<Dtype>* layer,
  ...
  layer->SetUp(*bottom, top);
  // printf("W:\n");
  // Blob<Dtype>* W = layer.blobs()[0].get();
  // for (int i = 0; i < W->count(); ++i) {
  //   printf("%.3f\t", W->cpu_data()[i]);
  //   if ((i+1)% W->width() == 0) printf("\n");
  // }
  // printf("\nb:%.3f\n", layer.blobs()[1]->cpu_data()[0]);
