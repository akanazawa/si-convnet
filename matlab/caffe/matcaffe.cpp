...
// get_blobs by Austin Myers 2014-04-14
//
...
static mxArray* do_get_blobs() {
  ...
  // Prepare output array of structures
  mxArray* mx_blobs;
  {
    const mwSize dims[2] = {num_blobs, 1};
    const char* fnames[2] = {"data", "names"};
    mx_blobs = mxCreateStructArray(2, dims, 2, fnames);
  }

  // Copy intermediate data into output
  for (unsigned int i = 0; i < blobs.size(); ++i) {
    // internally data is stored as (width, height, channels, num)
    mwSize dims[4] = {blobs[i]->width(), blobs[i]->height(),
                      blobs[i]->channels(), blobs[i]->num()};
    mxArray* mx_blob = mxCreateNumericArray(4, dims, mxSINGLE_CLASS,
                                            mxREAL);

    mxSetField(mx_blobs, i, "data", mx_blob);
    mxSetField(mx_blobs, i, "names",
               mxCreateString(blob_names[i].c_str()));

    float* blob_ptr = reinterpret_cast<float*>(mxGetPr(mx_blob));

    //  mexPrintf("layer: %s (%d) blob: %d  %d: (%d, %d, %d) %d\n",
    //  layer_names[i].c_str(), i, i, blobs[i]->num(),
    //  blobs[i]->height(), blobs[i]->width(),
    //  blobs[i]->channels(), blobs[i]->count());

    switch (Caffe::mode()) {
    case Caffe::CPU:
      memcpy(blob_ptr, blobs[i]->cpu_data(),
             sizeof(float) * blobs[i]->count());
      break;
    case Caffe::GPU:
      CUDA_CHECK(cudaMemcpy(blob_ptr, blobs[i]->gpu_data(),
                            sizeof(float) * blobs[i]->count(), 
...
static mxArray* do_get_ticonv_blobs() {
  const vector<shared_ptr<Layer<float> > >& layers = net_->layers();
  const vector<string>& layer_names = net_->layer_names();

  // Step 1: get the layer indices 
  vector<int> layer_ids;
  int num_layers = 0;
  {
    for (unsigned int i = 0; i < layers.size(); ++i) {
      vector<shared_ptr<Blob<float> > >& layer_blobs = layers[i]->blobs();
      if (layer_blobs.size() == 0 || layer_names[i].compare(0, 4, "conv") != 0 ) {
        continue;
      }
      TIConvolutionLayer<float>* ti_layer = dynamic_cast<TIConvolutionLayer<float>* >(layers[i].get());
      if (ti_layer != 0) { // we found a ti-layer!
	layer_ids.push_back(i);
      }
    }
    num_layers = layer_ids.size();
  }
  ...
  {
    const mwSize dims[2] = {num_layers, 1};
    const char* fnames[3] = {"outputs",  "max_switch", "layer_names"};
  ...
  // Step 3: copy weights into output
  {
    for (unsigned int i = 0; i < layer_ids.size(); ++i) {
      int id = layer_ids[i];
      TIConvolutionLayer<float>* ti_layer = dynamic_cast<TIConvolutionLayer<float>* >(layers[id].get());

      vector<shared_ptr<Blob<float> > >& activations = ti_layer->activations();
      Blob<float>& max_switch = ti_layer->max_switch();

      mxArray* mx_layer_cells = NULL;
      mxArray* mx_switch_cells = NULL;
      
      const mwSize dims[2] = {activations.size(), 1};
      mx_layer_cells = mxCreateCellArray(2, dims);
      mxSetField(mx_layers, i, "outputs", mx_layer_cells);

      const mwSize max_dims[2] = {1, 1};
      mx_switch_cells = mxCreateCellArray(2, max_dims);
      mxSetField(mx_layers, i, "max_switch", mx_switch_cells);
      mxSetField(mx_layers, i, "layer_names",
		 mxCreateString(layer_names[id].c_str()));
      // store  activations
      for (unsigned int j = 0; j < activations.size(); ++j) {
        // internally data is stored as (width, height, channels, num)
        // where width is the fastest dimension
        mwSize dims[4] = {activations[j]->width(), activations[j]->height(),
            activations[j]->channels(), activations[j]->num()};
        ...
        switch (Caffe::mode()) {
        case Caffe::CPU:
          memcpy(weights_ptr, activations[j]->cpu_data(),
              sizeof(float) * activations[j]->count());
          break;
        case Caffe::GPU:
          CUDA_CHECK(cudaMemcpy(weights_ptr, activations[j]->gpu_data(),
              sizeof(float) * activations[j]->count(), cudaMemcpyDeviceToHost));
          break;
        default:
          LOG(FATAL) << "Unknown caffe mode: " << Caffe::mode();
        }
      } // end of storing activations
      // store max-switch
      {
	mwSize dims[4] = { max_switch.width(), max_switch.height(),
			   max_switch.channels(), max_switch.num() };
	mxArray* mx_maxswitch = mxCreateNumericArray(4, dims, mxSINGLE_CLASS, mxREAL);
	mxSetCell(mx_switch_cells, 0, mx_maxswitch);
        float* maxswitch_ptr = reinterpret_cast<float*>(mxGetPr(mx_maxswitch));	
        switch (Caffe::mode()) {
        case Caffe::CPU:
          memcpy(maxswitch_ptr, max_switch.cpu_data(),
              sizeof(float) * max_switch.count());
          break;
        case Caffe::GPU:
          CUDA_CHECK(cudaMemcpy(maxswitch_ptr, max_switch.gpu_data(),
              sizeof(float) * max_switch.count(), cudaMemcpyDeviceToHost));
          break;
        default:
          LOG(FATAL) << "Unknown caffe mode: " << Caffe::mode();
        }	
      } // end of storing max-switch
    } // end of going over valid layers
  }
...
static void get_ti_blobs(MEX_ARGS) {
  plhs[0] = do_get_ticonv_blobs();
}
