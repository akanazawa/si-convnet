...
##############################
# CXX_SRC are the source files excluding the test ones.
...
all: $(NAME) $(STATIC_NAME) tools examples
	@echo "----$(foreach obj,$(EXAMPLE_OBJS),$(dir $(obj)))---"
	@echo "----${TOOL_BUILD_DIRS}---"
...
$(TOOL_BUILD_DIR)/%.o: tools/%.cpp $(PROTO_GEN_HEADER) | $(TOOL_BUILD_DIRS)
	$(CXX) $< $(CXXFLAGS) -c -o $@ 2> $@.$(WARNS_EXT) \
		|| (cat $@.$(WARNS_EXT); exit 1)
	@ cat $@.$(WARNS_EXT)
	@ echo
